...
void FrustumVisualizer::CreateDeviceResources(ID3D12Device2* device, DXGI_FORMAT rtFormat, DXGI_FORMAT dsFormat)
{
    ...
    ComPtr<ID3DBlob> meshShader2;
    ComPtr<ID3DBlob> pixelShader2;
    UINT compileFlags = D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION;



    if (FAILED(ReadDataFromFile(GetAssetFullPath(s_drawFrustumMsFilename).c_str(), &meshShader.data, &meshShader.size)))
    {
        ThrowIfFailed(D3DCompileFromFile(GetAssetFullPath(L"FrustumMS.hlsl").c_str(), nullptr, nullptr, "main", "vs_5_0", compileFlags, 0, &meshShader2, nullptr));
        meshShader.data = (byte*)meshShader2->GetBufferPointer();
        meshShader.size = (uint32_t)meshShader2->GetBufferSize();
    }
    if (FAILED(ReadDataFromFile(GetAssetFullPath(s_debugDrawPsFilename).c_str(), &pixelShader.data, &pixelShader.size)))
    {
        ThrowIfFailed(D3DCompileFromFile(GetAssetFullPath(L"DebugDrawPS.hlsl").c_str(), nullptr, nullptr, "main", "ps_5_0", compileFlags, 0, &pixelShader2, nullptr));
        pixelShader.data = (byte*)pixelShader2->GetBufferPointer();
        pixelShader.size = (uint32_t)pixelShader2->GetBufferSize();
    }
